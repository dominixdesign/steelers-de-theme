...
class leFtp {
  constructor(config) {
    ...
    else if (config.localRootDir && config.remoteRootDir)
      this.watchList = [
        {
    ...
    if (typeof ext == "undefined") {
      ...
      while ((c1 = ext.substr(0, 1) || true) && (c1 == "." || c1 == " "))
        ext = ext.substr(1);
      if (ext == "") extRegEx = "";
      else extRegEx = "\\.".concat(ext, "$");
    } else if (Array.isArray(ext)) {
      extRegEx = ext.reduce(function (regex, e) {
        while ((c1 = e.substr(0, 1) || true) && (c1 == "." || c1 == " "))
          e = e.substr(1);
    ...
    this.Ftp = new ftp();
    this.Ftp.on("greeting", (msg) => {
      console.log("FTP server found.");
    });
    this.Ftp.on("ready", () => {
  ...
  getSnapshot() {
    const walk = (localRoot, remoteRoot, path = "", structure = []) => {
      return new Promise((resolve, reject) => {
        let dirFullPath = localRoot.concat(
          localRoot.endsWith("/") || path == "" ? "" : "/",
          path
        );
        fs.readdir(dirFullPath, (err, files) => {
          if (err) console.log(`Error reading directory: ${dirFullPath}`);
            ...
            files.forEach((f) => {
              const filefullpath = `${dirFullPath}${
              ...
              fs.stat(filefullpath, (err, fstats) => {
                if (err) console.log(`${filefullpath} : Error ${err.code}`);
                  ...
                  walk(localRoot, remoteRoot, fileRelPath, structure).then(
                    (ignore) => {
                      if (!--fileToProcess) resolve(structure);
                    }
                  );
                else if (fstats.isFile()) {
                  if (this.extRegEx.test(f))
                    structure.push(
                      new sFile(
                        localRoot,
                        remoteRoot,
                        fileRelPath,
                        fstats.mtime.getTime()
                      )
                    );
                  if (!--fileToProcess) resolve(structure);
                }
    ...
    const loadDir = (i, allDirMap = []) => {
      return new Promise((resolve, reject) => {
        walk(
          this.watchList[i].localRootDir,
          this.watchList[i].remoteRootDir,
          "",
          []
        )
          .then((s) => {
            allDirMap = allDirMap.concat(s);
            if (i + 1 < this.watchList.length)
              loadDir(i + 1, allDirMap)
                .then((adm) => {
                  resolve(adm);
                })
                .catch((err) => {
  ...
  async uploadFile(file) {
    ...
    var remoteDirPath =
      file.remoteRoot + (file.remoteRoot == "" ? "" : "/") + localDirRelative;
    var remoteFileNameFull =
      file.remoteRoot + (file.remoteRoot == "" ? "" : "/") + file.name;
  ...
  deleteFile(file) {
    ...
    this.Ftp.delete(remoteFileNameFull, (err) => {
      if (err)
        console.log(`Cannot delete from remote file ${remoteFileNameFull}`);
